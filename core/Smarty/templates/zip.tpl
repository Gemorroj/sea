...
{block content}
    {if $action == 'preview'}
        <strong>{$language.file}: <a href="{$smarty.const.SEA_PUBLIC_DIRECTORY}zip/down/{Http_Request::get('id')}/{$zipFileName|rawurlencode|replace:'%2F':'/'}">{$zipFileName}</a></strong><br/>
        ...
        {if $zipFileType == 'text'}
            ...
            {* пагинация *}
            {paginationExtended page=$paginatorConf.page pages=$paginatorConf.pages url="{$smarty.const.SEA_PUBLIC_DIRECTORY}zip/preview/{Http_Request::get('id')}/{$zipFileName|rawurlencode|replace:'%2F':'/'}"}
        {/if}
    {else}
        ...
        {else}
            {foreach $zipFiles as $zipFile}
                <div class="{cycle values="row,row2"}">
                    <a href="{$smarty.const.SEA_PUBLIC_DIRECTORY}zip/preview/{Http_Request::get('id')}/{$zipFile.filename|rawurlencode|replace:'%2F':'/'}/">{$zipFile.filename}</a> ({$zipFile.size|sizeFormatExtended})<br/>
                </div>
            {/foreach}
        {/if}

        {* пагинация *}
        {paginationExtended page=$paginatorConf.page pages=$paginatorConf.pages url="{$smarty.const.SEA_PUBLIC_DIRECTORY}zip/{Http_Request::get('id')}"}
    {/if}
{/block}
