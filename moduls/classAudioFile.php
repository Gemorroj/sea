...
/* changes by Gemorroj (http://wapinet.ru)
2010-06-02
Small fixes
...
class AudioFile
{
    ...
    var $visual_fileformat; // "jpeg", "gif", "png", everything & else default = "png"
    var $frames = array(
        'AENC',
        'APIC',
        'ASPI',
    ...
    function mp3info()
    {
        ...
        if ($info['mpeg_bitrate'] == 'free' or $info['mpeg_bitrate'] == 'bad' or !$info['mpeg_bitrate'] or !$info['mpeg_sampling_rate']) {
            ...
            if ($byte_len == 0) {
                $rate_tmp = $info['mpeg_bitrate'] * 1024;
        ...
        $tmp2 = 0;
        $info['mpeg_frames'] = $info['mpeg_playtime'] = '';
        if (!$info['mpeg_bitrate'] or $info['mpeg_bitrate'] == 'bad' or !$info['mpeg_sampling_rate']) {
            $info['mpeg_playtime'] = -1;
        } else if ($info['mpeg_bitrate'] == 'free') {
            $info['mpeg_playtime'] = -1;
        } else {
            $tmp2 = ((8 * $tmp) / 1024) / $info['mpeg_bitrate'];
        ...
        if ($this->wave_length < 1 && is_array($this->id3v2->TLEN)) {
            $this->wave_length = ($this->id3v2->TLEN['value'] / 1024);
        }
        ...
        if ($this->wave_filename && $this->wave_id == 'RIFF' && $this->wave_type == 'WAVE' && ($this->wave_channels >= 1 && $this->wave_channels <= 2) && $this->wave_bits % 8 == 0) {
            ...
            if ($chunk_id_3 == 'fmt ') {
                ...
                if ($chunk_id_4 == 'data') {
                    ...
                    for ($j = 0, $allJ = sizeof($visualData); $j < $allJ; ++$j) {
                        $last_x = 1;
                        $last_y = $height_channel / 2;

                        // this for-loop draws the graphs itself

                        for ($i = 1, $allI = sizeof($visualData[$j]); $i < $allI; ++$i) {
                    ...
                    $format = strtolower($this->visual_fileformat);
                    if ($format == 'jpeg') {
                ...
                if (substr($chunk_id, 0, 2) == 'PK') {
                    ...
                    if ($this->wave_id == 'RIFF' && $this->wave_type == 'WAVE') {
                        ...
                        if ($this->wave_id == 'FORM' && $this->wave_type == 'AIFF') {
                            ...
                            $chunk_id = fgetc($file) . fgetc($file) . fgetc($file) . fgetc($file);
                            $chunk_size = $this->longCalc(fgetc($file), fgetc($file), fgetc($file), fgetc($file),
                                0);
                            if ($chunk_id == 'COMM') {
                                $format_len = $chunk_size;
    ...
    {
        echo '<table width="100%" border="1"><tr><td align="right">filename</td><td>' . htmlspecialchars($this->wave_filename, ENT_NOQUOTES) . '</td></tr>';
        echo '<tr><td align="right">id</td><td>' . htmlspecialchars($this->wave_id, ENT_NOQUOTES) . '</td></tr>';
        echo '<tr><td align="right">type</td><td>' . htmlspecialchars($this->wave_type, ENT_NOQUOTES) . '</td></tr>';
        echo '<tr><td align="right">size</td><td>' . htmlspecialchars($this->wave_size, ENT_NOQUOTES) . '</td></tr>';
        echo '<tr><td align="right">compression</td><td>' . htmlspecialchars($this->getCompression($this->wave_compression), ENT_NOQUOTES) . '</td></tr>';
        echo '<tr><td align="right">channels</td><td>' . htmlspecialchars($this->wave_channels, ENT_NOQUOTES) . '</td></tr>';
        echo '<tr><td align="right">framerate</td><td>' . htmlspecialchars($this->wave_framerate, ENT_NOQUOTES) . '</td></tr>';
        echo '<tr><td align="right">byterate</td><td>' . htmlspecialchars($this->wave_byterate, ENT_NOQUOTES) . '</td></tr>';
        echo '<tr><td align="right">bits</td><td>' . htmlspecialchars($this->wave_bits, ENT_NOQUOTES) . '</td></tr>';
        echo '<tr><td align="right">length</td><td>' . number_format($this->wave_length, 2) . ' sec.<br/>' . date('i:s', mktime(0, 0, round($this->wave_length))) . '</td></tr>';

        // ID3V1
        if ($this->id3_tag) {
            echo '<tr><td align="right">id3v1-tags</td><td>';
            echo '<table width="100%" border="1">';
            echo '<tr><td width="70" align="right">title</td><td>' . htmlspecialchars($this->id3_title, ENT_NOQUOTES) . '</td></tr>';
            echo '<tr><td align="right">artist</td><td>' . htmlspecialchars($this->id3_artist, ENT_NOQUOTES) . '</td></tr>';
            echo '<tr><td align="right">album</td><td>' . htmlspecialchars($this->id3_album, ENT_NOQUOTES) . '</td></tr>';
            echo '<tr><td align="right">year</td><td>' . htmlspecialchars($this->id3_year, ENT_NOQUOTES) . '</td></tr>';
            echo '<tr><td align="right">comment</td><td>' . htmlspecialchars($this->id3_comment, ENT_NOQUOTES) . '</td></tr>';
            echo '<tr><td align="right">genre</td><td>' . htmlspecialchars($this->id3_genre, ENT_NOQUOTES) . '</td></tr>';
            echo '</table>';
            echo '</td></tr>';
        } else {
            echo '<tr><td align="right">id3v1-tags</td><td>Not found</td></tr>';
        }

        // ID3V2
        if ($this->id3v2) {
            echo '<tr><td align="right">id3v2-tags</td><td>';
            echo '<table width="100%" border="1">';
            echo '<tr><td width="70" align="right">title</td><td>' . htmlspecialchars($this->id3v2->TIT2, ENT_NOQUOTES) . '</td></tr>';
            echo '<tr><td align="right">artist</td><td>' . htmlspecialchars($this->id3v2->TPE1, ENT_NOQUOTES) . '</td></tr>';
            echo '<tr><td align="right">original artist</td><td>' . htmlspecialchars($this->id3v2->TOPE, ENT_NOQUOTES) . '</td></tr>';
            echo '<tr><td align="right">album</td><td>' . htmlspecialchars($this->id3v2->TALB, ENT_NOQUOTES) . '</td></tr>';
            echo '<tr><td align="right">year</td><td>' . htmlspecialchars($this->id3v2->TYER, ENT_NOQUOTES) . '</td></tr>';
            echo '<tr><td align="right">comment</td><td>' . htmlspecialchars($this->id3v2->COMM, ENT_NOQUOTES) . '</td></tr>';
            echo '<tr><td align="right">composer</td><td>' . htmlspecialchars($this->id3v2->TCOM, ENT_NOQUOTES) . '</td></tr>';
            echo '<tr><td align="right">genre</td><td>' . htmlspecialchars($this->id3v2->TCON, ENT_NOQUOTES) . '</td></tr>';
            echo '<tr><td align="right">encoder</td><td>' . htmlspecialchars($this->id3v2->TENC, ENT_NOQUOTES) . '</td></tr>';
            echo '<tr><td align="right">website</td><td>' . htmlspecialchars($this->id3v2->WXXX, ENT_NOQUOTES) . '</td></tr>';
            echo '</table>';
            echo '</td></tr>';
        } else {
            echo '<tr><td align="right">id3v2 tags</td><td>Not found</td></tr>';
        }

        // VORBIS
        if ($this->wave_id == 'OGG') {
            echo '<tr><td align="right">ogg-tags</td><td>';
            echo '<table width="100%" border="1">';
            echo '<tr><td width="70" align="right">title</td><td>' . htmlspecialchars($this->vorbis_comment->TITLE, ENT_NOQUOTES) . '</td></tr>';
            echo '<tr><td align="right">artist</td><td>' . htmlspecialchars($this->vorbis_comment->ARTIST, ENT_NOQUOTES) . '</td></tr>';
            echo '<tr><td align="right">album</td><td>' . htmlspecialchars($this->vorbis_comment->ALBUM, ENT_NOQUOTES) . '</td></tr>';
            echo '<tr><td align="right">date</td><td>' . htmlspecialchars($this->vorbis_comment->DATE, ENT_NOQUOTES) . '</td></tr>';
            echo '<tr><td align="right">genre</td><td>' . htmlspecialchars($this->vorbis_comment->GENRE, ENT_NOQUOTES) . '</td></tr>';
            echo '<tr><td align="right">comment</td><td>' . htmlspecialchars($this->vorbis_comment->COMMENT, ENT_NOQUOTES) . '</td></tr>';
            echo '</table>';
            echo '</td></tr>';
        } else {
            echo '<tr><td align="right">ogg vorbis info</td><td>Not found</td></tr>';
        }
    ...
    function getId3v2(&$fp)
    {
        ...
        while (ftell($fp) < ($tagsize + 10)) {
            ...
            $frameid = fread($fp, 4);
            if (!in_array($frameid, $this->frames)) {
                continue;
            }
    ...
    function get32bitSynchsafe(&$fp)
    {
        ...
        */
        ...
        return ($tmp * 2097152) + ($tmp2 * 16384) + ($tmp3 * 128) + $tmp4;
    }
    ...
    function ogginfo()
    {
        ...
        while ($type < 4 && !feof($fp)) {
            list($type, $fp) = $this->findOggPage($fp);
        }
        ...
        */
        ...
        $this->wave_byterate = ($identification['bitrate_nominal'] / 1024) . ' Kbit/sec';
        $this->wave_bits = 'n/a';
