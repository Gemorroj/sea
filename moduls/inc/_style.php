...
if ($setup['style_change']) {
    if (isset($_POST['style']) && parse_url($_POST['style']) && strpos($_POST['style'], '.')) {
        ...
        setcookie('style', $style, $_SERVER['REQUEST_TIME'] + 2592000, DIRECTORY, $_SERVER['HTTP_HOST']);
    } else if (isset($_GET['style']) && parse_url($_GET['style']) && strpos($_GET['style'], '.')) {
        $style = ltrim($_GET['style'], 'http://');
        setcookie('style', $style, $_SERVER['REQUEST_TIME'] + 2592000, DIRECTORY, $_SERVER['HTTP_HOST']);
    } else if (isset($_COOKIE['style']) && parse_url($_COOKIE['style']) && strpos($_COOKIE['style'], '.')) {
